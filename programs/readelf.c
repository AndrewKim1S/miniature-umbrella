...
//--------------------------------------------- Inline Library Call (getopt_long)
int opterr = 1;
int optind = 1;
int optopt = '?';
char *optarg;
...
static const char recargchar[] = "option requires and argument -- %c";
...
static int
parse_long_options(char * const *nargv, const char *options, const struct option *long_options, int *idx, int short_too) {
...
int getopt_long_custom(int nargc, char * const *nargv, const char *options, const struct option *long_options, int *idx) {
...
parse_args (int argc, char ** argv)
{
  ...
  // inline getopt_long --> getopt_long_custom
  // while ((c = getopt_long(argc, argv, "ADHINR:SVWacdeghi:lnp:rstuvw::x:z", options, NULL)) != EOF) {
  while ((c = getopt_long_custom(argc, argv, "ADHINR:SVWacdeghi:lnp:rstuvw::x:z", options, NULL)) != EOF) {
      switch (c)
	{
	case 0:
	  /* Long options.  */
...
get_file_header (FILE * file)
{
  ...
  else       // <-- else statement is taken for CVE poc
    {
...
// ------------------------------------------------------ inline fopen
// This fopen implementation was found from glib/libio/iofopen.c
__attribute__((noinline)) FILE *fopen_custom(const char *path, const char *mode) {
  int flags = 0;
  int fd;
...
process_file (char * file_name)
{
  ...
  */
  ...
  file = fopen_custom (file_name, "rb");
