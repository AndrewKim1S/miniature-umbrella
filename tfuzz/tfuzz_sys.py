...
class TFuzzSys():
    ...
    def __init__(self, binary_path, workdir, afl_count=1, target_opts=None,
                 ...
                 input_placeholder='@@', afl_opts=None, test_mode=True):
        ...
        # setup the NCCDetector
        # import ipdb; ipdb.set_trace()
        if test_mode:
            ff = FuncBasedFilter(self.origin_binary_path, include_funcs=['main'])
        else:
            ff = FuncBasedFilter(self.origin_binary_path, exclude_funcs=['main'])
    ...
    def __fuzz_one_program(self):
        ...
        if self.__current_fuzzer.failed_to_start():
            ...
            print("%s failed to start" % str(self.__current_fuzzer))
            return False
    ...
    def run(self):
        ...
        while not self.__stop:
            ...
            if not self.__fuzz_one_program():
                logger.warn("%s failed to start" % (self.fuzzing_program))
                continue

            print("Fuzzing %s" % (self.fuzzing_program.program_name))
            while not self.__current_fuzzer.is_stuck():
                time.sleep(self.check_interval)


            print("Fuzzer got stuck")
            ...
            if crash_seeds > 0 or crash_found > 0:
                print("Crashes found while fuzzing %s" % (self.fuzzing_program.program_name))
            ...
            for from_addr, to_addr in self.nccdetector.detect_nccs(acc_cov):                
                ...
                # logger.info("== (%s, %s) == going to be disabled" % (hex(from_addr), hex(to_addr)))
                # print("== (%s, %s) == going to be forced" % (hex(from_addr), hex(to_addr)))
