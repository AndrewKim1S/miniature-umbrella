...
class CrashAnalyzer(object):
    ...
    def __init__(self, tprogram, input_file, input_placeholder='@@', target_opts=None, seed=None):
        ...
        if target_opts == None or input_placeholder not in target_opts:
            ...
            self.target_opts = [os.path.abspath(self.tprogram.program_path)] + \
                               self.target_opts
            # DEBUGGING
            #print("===== Running QEMURunner with target program and target args =====")
            #print("binary: ", os.path.abspath(tprogram.program_path))
            #print("input: ", file(input_file).read())
            #print("seed: ", seed)
            #print("argv: ", target_opts)
    ...
    def collect_constraints(self):
        ...
        # DEBUGGING
        print("\n==================== PRINTING TRACE ====================")
        #print(self.r.trace) # This just prints out basic block address
	for addr in self.r.trace: 
	    block = self.p.factory.block(addr)
        ...
        print("\n======== PRINTING CONSTRAINTS SELECTED BY ANGR =========")
        self.simgr.run()
        print("========================================================\n")
